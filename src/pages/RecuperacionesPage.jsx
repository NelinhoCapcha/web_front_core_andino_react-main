...
export default function RecuperacionesPage() {
  ...
  useEffect(() => {
    function onMoraRefresh() {
      recargarResumen()
      recargarCartera()
    }
    function onStorage(e) {
      if (e.key === 'qapaq_mora_refresh') onMoraRefresh()
    }
    window.addEventListener(MORA_REFRESH_EVENT, onMoraRefresh)
    window.addEventListener('storage', onStorage)
    return () => {
      window.removeEventListener(MORA_REFRESH_EVENT, onMoraRefresh)
      window.removeEventListener('storage', onStorage)
    }
  }, [recargarResumen, recargarCartera])
  ...
  async function ejecutarTransicion(c, tipo) {
    ...
    try {
      ...
      refrescar()
      notificarCambioMora({ codcuentacredito: c.codcuentacredito, estado: res.estado })
    } catch (err) {
