...
export function useGestiones(cod) {
  ...
  const eliminar = useCallback(
    async (pkgestion) => {
      ...
      setAccionMsg(null)
      try {
        const res = await eliminarGestion(cod, pkgestion)
        setAccionMsg({ tipo: 'ok', texto: 'Gestion eliminada correctamente.' })
        await cargar()
        return res
      } catch (err) {
        setAccionMsg({ tipo: 'error', texto: detalleError(err, 'No se pudo eliminar la gestion.') })
        return null
      } finally {
        setAccionLoading(false)
      }
  ...
  return { gestiones, loading, error, recargar: cargar, registrar, eliminar, accionLoading, accionMsg }
}

/** Historial R3 de transiciones de estado de un credito. */
export function useHistorialEstados(cod) {
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null)
    try {
      setHistorial(await getHistorialEstados(cod))
    } catch (err) {
      setError(detalleError(err, 'No se pudo cargar el historial de estados.'))
    } finally {
      setLoading(false)
    }
  }, [cod])
